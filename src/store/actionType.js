export const TOGGLE_DRAWER = 'TOGGLE_DRAWER';
export const CLOSE_DRAWER = 'CLOSE_DRAWER';
export const INIT_SETUP = 'INIT_SETUP';
export const GET_LOOK = 'GET_LOOK';
export const FILTER_LOOKS = 'FILTER_LOOKS';
export const SORT = 'SORT';
export const SLIDE = 'SLIDE';
export const FILTER_APPLIED = 'FILTER_APPLIED';
export const FILTER_REMOVED = 'FILTER_REMOVED';
export const ADDTOFAV = 'ADDTOFAV';
export const REMOVEFAV = 'REMOVEFAV';
export const ADDTOBAG = 'ADDTOBAG';
export const REMOVEFROMBAG = 'REMOVEFROMBAG';
export const AUTOFETCHFAVBAG = 'AUTOFETCHFAVBAG';
export const SETFAVONLOOKS = 'SETFAVONLOOKS';
export const BAGNOTIFICATION = 'BAGNOTIFICATION';
export const BAGNOTIFICATIOFF = 'BAGNOTIFICATIOFF';
export const ADMINLOGGEDIN = 'ADMINLOGGEDIN';
export const ADMINLOGOUT = 'ADMINLOGOUT';